...
public class SceneObject : MonoBehaviour {
 ...
 //   void Start () {
    ...
    // 操作监听返回
    public delegate void Listen_Operate_Ret(string typ, bool result);
    public Listen_Operate_Ret listenOperate;
    public void listen_operate_result(Int64 userid, string typ, string result)
    {
        Debug.Log(userid + " " + typ + " " + result);
        Debug.Log(UserInfor.getInstance().UserId);

        if (UserInfor.getInstance().UserId != userid)
        {
        ...
        if (listenOperate != null)
        {
            listenOperate.Invoke(typ,sos.isCanOperate);
        }
    }
