...
public class DrawPadRT : MonoBehaviour
{
    ...
    public ToolType currenttooltyp = ToolType.None;
    ...
    void Start()
    {
        ...
        //textureW = rect.localScale.x * textureScale;
        //textureH = rect.localScale.z * textureScale;

        //Resolution resolution = Screen.currentResolution;
        //scalex = resolution.width / 2;
        //scaley = resolution.height / 2;
        float wh = 5 * (eye.pixelWidth / eye.pixelHeight);
        scaley = eye.pixelHeight / 5;
        scalex = eye.pixelWidth / wh;

        Debug.Log(eye.pixelWidth + " " + eye.pixelHeight);
        Debug.Log(scaley + " " + scaley);

        RectTransform rect = gameObject.GetComponent<RectTransform>();
        textureW = 10;// rect.sizeDelta.x;// * textureScale;
        textureH = 10;// rect.sizeDelta.y;// * textureScale;
        ...
        rtSource = new RenderTexture((int)(1*scalex), (int)(1*scaley), 0, RenderTextureFormat.ARGB32);
        rtSource.wrapMode = TextureWrapMode.Clamp;

        // 测试
        lefttop = new Vector3(transform.position.x, transform.position.y + (int)(1 * scaley) / 2, transform.position.z);
        leftbuttom = new Vector3(transform.position.x, transform.position.y - (int)(1 * scaley) / 2, transform.position.z);
        righttop = new Vector3(transform.position.x + (int)(1 * scalex), transform.position.y + (int)(1 * scaley) / 2, transform.position.z);
        rightbuttom = new Vector3(transform.position.x + (int)(1 * scalex), transform.position.y - (int)(1 * scaley) / 2, transform.position.z);
    ...
    void Update()
    {
        ...
        Debug.DrawLine(lefttop, leftbuttom);
        Debug.DrawLine(lefttop, righttop);
        Debug.DrawLine(righttop, rightbuttom);
        Debug.DrawLine(rightbuttom, leftbuttom);
        if (Input.GetKeyDown(KeyCode.P))
        {
            Debug.Log(c.localPosition);
            OnPressDownDrawPad(c.position);
            b = true;
        }

        if (b)
        {
            OnDrawing(c.position);
        }
    ...
    public void OnPressDownDrawPad(Vector3 pos)
    {
        pos = transform.InverseTransformPoint(pos);
        pos.x = (textureW / 2 - (pos.x - transform.position.x));
        pos.y = (textureH / 2 - (pos.z - transform.position.z));
        KeepForUndo();
    ...
    public void OnDrawing(Vector3 pos)
    {
        pos = transform.InverseTransformPoint(pos);
        pos.x = (textureW /2 - (pos.x - transform.position.x));
        pos.y = (textureH / 2 - (pos.z - transform.position.z));
    ...
    void PaintOnPos(Vector2 targetPos)
    {
        ...
        if (currentTouchPos != previousTouchPos)
        {
            ...
            Debug.Log(segmentDistance + "segmentDistance");
    ...
    void ColorThePoint(Vector2 pos)
    {
        ...
        GL.PushMatrix();
        GL.LoadPixelMatrix(0, (int)(textureW * scalex), (int)(textureH * scaley), 0);
        Graphics.DrawTexture(new Rect(pos.x - currentBrushSize / 2, (int)(textureH * scaley) - pos.y - currentBrushSize / 2, currentBrushSize, currentBrushSize), brush, brushTexMat);
        GL.PopMatrix();
    ...
    {
        if (currenttooltyp != ToolType.Pen)
        {
            currenttooltyp = ToolType.Pen;
    ...
    {
        if (currenttooltyp != ToolType.Eraser)
        {
            currenttooltyp = ToolType.Eraser;
